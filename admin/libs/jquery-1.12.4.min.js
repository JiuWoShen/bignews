...
!(function(a, b) {
 ...
 //这里写$.ajaxSetup();-------------写入jQuery js 文件------这样只要调用了这个文件就-------让后面每次  ajax请求   都可以   携带token令牌 
//  $.ajaxSetup({name:value, name:value, ... })------ajaxSetup() 方法为将来的 AJAX 请求设置默认值----------参数---beforeSend(xhr)	发送请求前运行的函数
  $.ajaxSetup({
      //发送请求前运行的函数。
      ...
      //如果请求失败要运行的函数---------error(xhr,status,error)-------	如果请求失败要运行的函数。
